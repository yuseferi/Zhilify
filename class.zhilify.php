<?php
/**
 * Zhilify: Structured information display solution
 *
 * Zhilify is a debugging tool (PHP5 only), which displays structured information
 * about any PHP variable. It is a nice replacement for print_r() or var_dump()
 * which are used by a lot of PHP developers.
 *
 * @original author Zhilevan <zhilevan@gmail.com>
 * @license http://opensource.org/licenses/lgpl-license.php GNU Lesser General Public License Version 2.1
 *
 * https://github.com/mmucklo/zhilify
 */
 ...
 * Zhilify API
 *
 * This class stores the Zhilify API for rendering and
...
class Zhilify
{
    ...
    const ZHILIFY_RETURN = 0;
    const ZHILIFY_EXPAND_ALL = 1;
    const ZHILIFY_SORT = 2;

    /**
     * Return Zhilify version
     *
     * @return string
     * @access public
     * @static
    ...
    public static function dump($data, $second = '')
    {
        ...
        } elseif ($second === self::ZHILIFY_EXPAND_ALL) {
            static::$expand_all = true;
            static::dump($data);

            return true;
        } elseif ($second === self::ZHILIFY_SORT) {
            self::$sort = true;
            Zhilify::dump($data);
        ...
        while ($d = array_pop($_)) {
            ...
            if (in_array($function, array("zhilify", "zh", "zhd")) || (strToLower(@$d['class']) == 'zhilify') || (is_callable($callback) && call_user_func($callback, $d))) {
                break;
            }
        }
        $showVersion = static::_config('display', 'show_version', true);
        $showCallInfo = static::_config('display', 'show_call_info', true);
        $zhilifyUrl = 'https://github.com/zhilevan/zhilify';

        //////////////////////
        // Start HTML header//
        //////////////////////
        print "<div class=\"zhilify-root\">\n";
        print "\t<ul class=\"zhilify-node zhilify-first\">\n";

        // The actual item itself
        static::_dump($data);

        if ($showVersion || $showCallInfo) {
            print "\t\t<li class=\"zhilify-footnote\" onDblClick=\"toggle_expand_all();\">\n";

            if ($showCallInfo && isset($d['file']) && $d['file']) {
                print "<span class=\"zhilify-call\" style=\"white-space:nowrap;\">";
            ...
            if ($showVersion) {
                ...
                print "<span class=\"zhilify-version\" style=\"white-space:nowrap;\">\n";
                print "<strong class=\"zhilify-version-number\">Zhilify version $version</strong> | <a href=\"$zhilifyUrl\" target=\"_blank\">$zhilifyUrl</a>\n";
                print "</span>\n";
            }
    ...
    /**
     * Returns values from Zhilify's configuration
     *
     * @param string $group
     * @param string $name
    ...
    {
        $zhilify_ini = dirname(__FILE__) . DIRECTORY_SEPARATOR . 'zhilify.ini';

        // The config isn't loaded yet
        if (empty(static::$_config) && is_readable($zhilify_ini)) {
            static::$_config = (array)parse_ini_file($zhilify_ini, true);
        }
    ...
    private static function _css()
    {
        ...
        $css = '';
        $skin = static::_config('skin', 'selected', 'default');
        // custom selected skin
        $rel_css_file = "skins/{$skin}/skin.css";
        $css_file = dirname(__FILE__) . DIRECTORY_SEPARATOR . $rel_css_file;
        ...
        if ($_css = $css != '') {
            ...
            $relative_zhilify_path = static::calculate_relative_path(__FILE__, true);
            $css_url = static::_config('css', 'url', $relative_zhilify_path);

            // Default to /zhilify/ if nothing is found in the config
            $css_url || $css_url = "/zhilify/";
            ...
            // the CSS
            print "<!-- Using Zhilify Skin: \"$skin\" $rel_css_file -->\n";
            ...
            $js_file = dirname(__FILE__) . DIRECTORY_SEPARATOR . "/js/zhilify.min.js";
            if (is_readable($js_file)) {
                $js_text = file_get_contents($js_file);
            } else {
                $js_text = "// Missing JS file zhilify.min.js\n";
            }

            print "$js_text</script>\n";
            print "<!-- Zhilify - JavaScript -->\n";
        }
    ...
    /**
     * Get\Set Zhilify state: whether it is enabled or disabled
     *
     * @param boolean $state
     * @return boolean
     * @access private
...
//            static::_closure();
    ...
    {
        $html = '<li class="zhilify-child">
            <div class="zhilify-element" onMouseOver="zhilify.over(this);" onMouseOut="zhilify.out(this);">
            <a class="zhilify-name">%s</a> %s <em class="zhilify-type zhilify-null">NULL</em>
            </div></li>';
    ...
    private static function _marker()
    {
        ...
        if (!isset($_recursion_marker)) {
            $_recursion_marker = uniqid('zhilify');
        }
    ...
    private static function _vars(&$data)
    {
        ...
        print "<div class=\"zhilify-nest\" $collapse_style>";
        print "<ul class=\"zhilify-node\">";
    ...
    {
        $html = '<div class="zhilify-nest" style="display:none;">
            <ul class="zhilify-node">
                <li class="zhilify-child">
                    <div class="zhilify-element" onMouseOver="zhilify.over(this);" onMouseOut="zhilify.out(this);">
                        <a class="zhilify-name">&#8734;</a>
                        (<em class="zhilify-type">Recursion</em>)
                    </div>
    ...
    private static function _array($data, $name)
    {
        if (self::$sort === false) {
            ...
            $config_sort = static::_config('sorting', 'sort_arrays', false);
        }
        ...
        if ($childCount > 0 && $collapsed) {
            $elementClasses = ' zhilify-expand';
        } elseif ($childCount > 0) {
            $elementClasses = ' zhilify-expand zhilify-opened';
        } else {
            $elementClasses = '';
        }

        print "<li class=\"zhilify-child\">";
        print "<div class=\"zhilify-element $elementClasses\"";

        // If there is more than one, make a dropdown
        if (count($data) > 0) {
            print "onClick=\"zhilify.toggle(this);\"";
        }

        print "onMouseOver=\"zhilify.over(this);\" onMouseOut=\"zhilify.out(this);\">";
        print "<a class=\"zhilify-name\">$name</a> <em class=\"zhilify-type\">Array(<strong class=\"zhilify-array-length\">";
        ...
        if ($sort) {
            $title = "Array has been sorted prior to display. This is configurable in zhilify.ini.";
            print " - <span title=\"$title\"><strong class=\"zhilify-sorted\">Sorted</strong></span>";
        }

        // callback
        if (is_callable($data)) {
            $_ = array_values($data());
            print "<span class=\"zhilify-callback\"> |";
            print " (<em class=\"zhilify-type\">Callback</em>) <strong class=\"zhilify-string\">";
    ...
    private static function _object(&$data, $name)
    {
        ...
        print "<li class=\"zhilify-child\"> <div class=\"zhilify-element $elementClasses\"";
        if (count($data) > 0) {
            print 'onClick="zhilify.toggle(this);"';
        }
        print 'onMouseOver="zhilify.over(this);" onMouseOut="zhilify.out(this);">';
        ...
        print "<a class=\"zhilify-name\">$name</a> <em class=\"zhilify-type\">Object</em> ";
        print static::get_separator() . " <strong class=\"zhilify-class\">$class_name</strong>$empty_str</div>";
    ...
    private static function _resource($data, $name)
    {
        $html = '<li class="zhilify-child">
            <div class="zhilify-element" onMouseOver="zhilify.over(this);" onMouseOut="zhilify.out(this);">
            <a class="zhilify-name">%s</a> <em class="zhilify-type">Resource</em>
            %s<strong class="zhilify-resource">%s</strong>
            </div></li>';
    ...
    private static function _boolean($data, $name)
    {
        ...
        $html = '<li class="zhilify-child">
            <div class="zhilify-element" onMouseOver="zhilify.over(this);" onMouseOut="zhilify.out(this);">
            <a class="zhilify-name">%s</a> <em class="zhilify-type">Boolean</em>
            %s<strong class="zhilify-boolean">%s</strong>
            </div></li>';
    ...
    {
        print "<li class=\"zhilify-child\">";
        print "<div class=\"zhilify-element\" onMouseOver=\"zhilify.over(this);\" onMouseOut=\"zhilify.out(this);\">";
        print "<a class=\"zhilify-name\">$name</a> <em class=\"zhilify-type\">Integer</em> ";
        print static::get_separator() . " <strong class=\"zhilify-integer\">$data</strong>";

        $ut = static::is_datetime($name, $data);
        if ($ut) {
            print " ~ <strong class=\"zhilify-datetime\">$ut</strong>";
        }
    ...
    private static function _float($data, $name)
    {
        print "<li class=\"zhilify-child\">";
        print "<div class=\"zhilify-element\" onMouseOver=\"zhilify.over(this);\" onMouseOut=\"zhilify.out(this);\">";
        print "<a class=\"zhilify-name\">$name</a> <em class=\"zhilify-type\">Float</em> ";
        print static::get_separator() . " <strong class=\"zhilify-float\">$data</strong>";
    ...
    private static function _string($data, $name)
    {
        ...
        if ($display_cr) {
            $_ = preg_replace("/(\\r\\n|\\n|\\r)/", "<strong class=\"zhilify-carrage-return\"> &para; </strong>", $_);
        } else {
            $_ = nl2br($_);
        }
        ...
        $expand_class = '';
        if ($_extra) {
            $expand_class = 'zhilify-expand';
        }

        print "<li class=\"zhilify-child\">";
        print "<div class=\"zhilify-element $expand_class\" ";
        if ($_extra) {
            print " onClick=\"zhilify.toggle(this);\" ";
        }
        print "onMouseOver=\"zhilify.over(this);\" onMouseOut=\"zhilify.out(this);\">\n";

        print "<a class=\"zhilify-name\">$name</a> ";
        print "<em class=\"zhilify-type\">String(<strong class=\"zhilify-string-length\">" . strlen($data) . "</strong>)</em> $icon";

        print static::get_separator() . " <strong class=\"zhilify-string\">" . $_;
        ...
        if (is_callable($data)) {
            print "<span class=\"zhilify-callback\"> | ";
            print "(<em class=\"zhilify-type\">Callback</em>) <strong class=\"zhilify-string\">" . htmlSpecialChars($_) . "()</strong></span>";
        }
        ...
        if ($_extra) {
            ...
            print "<div class=\"zhilify-nest\" $collapse_style>";
            print "<ul class=\"zhilify-node\">";
            print "<li class=\"zhilify-child\"> <div class=\"zhilify-preview\">" . $data . "</div></li>";
            print "</ul></div>";
        }
    ...
    private static function cli_dump()
    {
        ...
        foreach ($args as $i) {
            ...
            if (sizeof($args) >= 1) {
                $version = static::version();
                print "\n\nCalled from $file, line $line  (Zhilify version $version)\n$bar\n";
            }
        }
    }
}
 ...
 * @see static::dump()
 */
if (!function_exists("zhilify")) {
    function zhilify()
    {
        $_ = func_get_args();
        print_r($_);

        return call_user_func_array(array('zhilify', 'dump'), $_);
    }
}
...
if (!function_exists('tb')) {
    ...
    {
        $_ = func_get_args();
        if(stristr($_[1],'expand') || stristr($_[1],'e') ){
            $_['second'] = zhilify::ZHILIFY_EXPAND_ALL;
            unset($_[1]);
        }
        return call_user_func_array(array('zhilify', 'dump'), $_);
    }
...
if (!function_exists('tbd')) {
    function zhd()
    {
        ...
        call_user_func_array(array('zhilify', 'dump'), $_);
        exit();
    }
}

if (!function_exists('tbs')) {
    function zhs()
    {
        if (php_sapi_name() !== 'cli') {
            Zhilify::htmlHeaders();
